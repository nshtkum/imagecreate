...
def generate_ai_image(prompt, output_size=(1200, 675)):
    """Generate image using Hugging Face API with specific dimensions"""
    if not client:
        ...
        with st.spinner(f"🎨 Creating your property image{'s' if len(sizes_to_generate) > 1 else ''}..."):
            try:
                ...
                with st.expander("📊 Generation Summary"):
                    ...
                    st.markdown(f"**🖼️ Source:** {'AI Generated' if image_source == '🤖 Generate with AI' else 'Uploaded Image'}")
